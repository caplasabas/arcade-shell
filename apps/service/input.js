...
const INTERNET_RESTORE_THRESHOLD = 1
...
let depositPulseGaps = [] // Track gaps between pulses for diagnostics
...
function scheduleRetroarchReadyWatch(onReady) {
  ...
  const tick = () => {
    ...
    if (fs.existsSync(RETROARCH_READY_FILE)) {
      clearRetroarchReadyWatch()
      try {
        onReady()
      } catch (error) {
...
function startArcadeTimeLoop() {
  ...
  arcadeTimeLoopTimer = setInterval(async () => {
    ...
    if (arcadeSession.arcadeTimeMs <= 0) {
      ...
      // console.log('[ARCADE TIME] exhausted → exiting')
      //
      // // if (typeof exitRetroarch === 'function') {
      // //   exitRetroarch()
      // // }
    }
...
async function fetchDeviceBalanceSnapshot() {
  ...
  const url =
    `${SUPABASE_URL}/rest/v1/device_stats_live?` +
  ...
  if (buyState === 'confirm') {
    ...
    try {
      ...
      if (Number.isFinite(nextTimeMs)) {
        ...
        // fallback (only if backend not deployed yet)
      ...
      // Balance already came from the purchase RPC; only refresh credit here.
...
function handleDepositPulse() {
  ...
  if (depositPulseCount === 0) {
    ...
    depositPulseGaps = [] // Reset gaps on new deposit
    console.log('\n[DEPOSIT] START')
  }
...
let networkConfirmed = false // Wait until network is confirmed before updating latency
const LATENCY_UPDATE_INTERVAL_MS = 60000 // Update DB every 60 seconds
...
async function checkInternetReachability() {
  try {
      ...
      if (internetOkCount >= INTERNET_RESTORE_THRESHOLD) {
        if (internetLastStableState !== 'ok') {
          ...
          internetDebounceTimer = setTimeout(() => {
            ...
            // Re-enable coins when transitioning to OK (including boot from unknown)
    ...
    if (internetLastStableState !== 'offline' && Date.now() >= internetBootGraceUntil) {
      ...
      // Immediately inhibit coins on network failure (after boot grace)
      setCoinInhibit(true)
...
// Initially assume coins should be inhibited until first network check completes
// At boot we unconditionally inhibit, so track that state
setCoinInhibit(true)
coinInhibitedByNetwork = true
...
const inputDeviceStates = new Map()
...
function describeInputPath(path) {
  try {
...
function getInputDeviceState(path, label) {
...
function startEventDevice(path, label) {
  ...
  if (waitingInputDevices.delete(path)) {
    console.log(`[READY] ${label} detected`, describeInputPath(path))
  ...
  fs.open(path, 'r', (err, fd) => {
    if (generation !== state.generation) {
      state.opening = false
      if (!err && Number.isInteger(fd)) fs.close(fd, () => {})
      return
    }
        ...
        if (err || bytesRead !== 24) {
          ...
          const retryDelay =
            typeof errorMessage === 'string' && errorMessage.includes('ENODEV')
              ? INPUT_DEVICE_RETRY_ENODEV_MS
              : INPUT_DEVICE_RETRY_ERROR_MS
...
function handleKey(source, index, value) {
  ...
  if (arcadeSession?.active && player) {
    ...
    // 🚫 GLOBAL TIME LOCK (soft lock)
    // Blocks gameplay when time = 0, but still allows buying more time or exiting.
  ...
  if (retroarchActive) {
    ...
    if (arcadeSession?.active) {
      ...
      if (isStartButton(index)) {
        ...
        if (handleSimultaneousRetroarchStart(player, target, value)) {
          return
        }
